...
nrsf <- TF.benchmark.corrected$nrsf[["nrsf/112"]]
counts <- with(subset(nrsf$coverage, strand=="both"), {
  data.frame(sample.id=paste(experiment, sample.id),
...
for(sample.id in names(profile.list)){
  ...
  sample.regions <- data.frame(sample.id, nrsf$corrected.regions)
  if(grepl("backgr", sample.id)){
    sample.regions$annotation <- "noPeaks"
  }
  region.list[[sample.id]] <- sample.regions
}
...
for(sample.id in names(profile.list)){
  ...
  if(nrow(peak.df)){
    peak.list[[sample.id]] <-
      data.frame(sample.id, peak.df, y)
  }
...
ggplot()+
  ...
  theme(panel.margin=grid::unit(0, "cm"))+
  coord_cartesian(xlim=c(56406.4, 56408.5))+
  facet_grid(sample.id ~ ., scales="free", labeller=function(var, val){
    sub(" ", "\n", val)
  })

png("figure-nrsf-profiles.png", width=9, h=7, res=200, units="in")
print(problemsPlot)
dev.off()
...
print(single.times.xt, include.rownames=FALSE, floating=FALSE,
      file="table-nrsf-PeakSeg.tex")

xt <- xtable(best.times.df)
print(xt, file="table-nrsf.tex", floating=FALSE)
