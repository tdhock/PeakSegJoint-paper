...
\author{Toby Dylan Hocking toby.hocking@mail.mcgill.ca \\
...
back in to the BinSize=1 level we return the final Peak positions, and
the $p$ Samples in which that peak was observed.

\begin{figure}[b!]
  \centering
  \includegraphics[width=0.95\textwidth]{figure-heuristic-algo}
  \vskip -0.5cm
...
We used McGill benchmark data sets from
\url{http://cbio.ensmp.fr/~thocking/chip-seq-chunk-db/} which included
a total of 12,826 manually annotated region labels
\citep{hocking2014visual}. Each data set contains labels
grouped into windows of nearby regions (from 4 to 30 windows per data
set). For each data set, we performed 6 random splits of windows into
half train, half test. Since there are multiple peaks per window, and
\ref{PeakSegJoint} can detect at most 1 peak per sample, we divided
each window into separate \ref{PeakSegJoint} problems of size $B$ (a
...
\ref{PeakSeg} model. In contrast, the baseline hmcan.broad and macs
models from the bioinformatics literature were each only effective for
a single experiment type (either H3K36me3 or H3K4me3, but not both).
...
% and PePr were unable to analyze the McGill benchmark data sets.
...
\JointHeuristic\ with the $O(B\log B)$ unconstrained pruned dynamic
...
one peak. Figure~\ref{fig:timings} indicates that the
\JointHeuristic\ algorithm of \ref{PeakSegJoint} enjoys the same $O(B
\log B)$ asymptotic behavior as the pDPA. However the \JointHeuristic\
...
The table below presents a summary of our proposed \ref{PeakSegJoint}
model alongside several other peak detection algorithms. Time
complexity is for detecting one peak in $B$ data points. The ``?''  of
the JAMM and PePr unsupervised joint peak detectors means that their
articles did not discuss the time complexity \citep{JAMM, PePr}.

\begin{center}
  \begin{tabular}{crrcc}
  Algorithm & Samples & Cell types & Time complexity & Learning \\
  \hline
  JAMM & $\geq 1$ & 1 & ? & unsupervised \\
  PePr & $\geq 2$ & 1--2 & ? & unsupervised \\
  PeakSeg (cDPA) & 1 & 1 & $O(B^2)$ & supervised \\
  PeakSegJoint (JointZoom) & $\geq 1$ & $\geq 1$ & $O(B \log B)$ & supervised
\end{tabular}
\end{center}
...
algorithms. The \ref{PeakSegJoint} model is more
