...
analysis of three or more cell types. In contrast, our proposed
\ref{PeakSegJoint} method models data from several samples without
...
experiments \citep{jmosaics,segway,chromhmm}. In these models, the
input is one sample (e.g. monocyte cells) with different experiments
such as H3K4me3 and H3K36me3 profiles. Also,
\citet{hierarchical-joint} proposed a model for one sample with both
ChIP-seq and ChIP-chip data. In contrast, our proposed
\ref{PeakSegJoint} model takes as input several samples
...
show results for two unsupervised single-sample %and multi-sample
