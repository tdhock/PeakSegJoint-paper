...
vector $\mathbf w\in\RR^d$ in an affine function $f(\mathbf X) =
1^\intercal \mathbf X \mathbf w$.
...
y_i = \left[
  ...
  \end{array}
\right]\in\RR^2$ of penalty values. We used the algorithm of
...
X_i)\in[\underline y_i,\, \overline y_i]$ implies that the
