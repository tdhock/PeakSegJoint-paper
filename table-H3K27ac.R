...
load("H3K27ac_TDH_control.RData")

chunk.data <- H3K27ac_TDH_control[[1]]
convertSample <- function(df){
  df$sample <- df$sample.id
  df$experiment <- ifelse(grepl("Input", df$sample.type), "Input", "H3K27ac")
  df$sample.id <- with(df, paste(experiment, sample.id))
  subset(df, sample %in% c(
    "McGill0012", "McGill0015",
    "CEMT_44", "CEMT_45"
    ))
}
counts <- convertSample(chunk.data$counts)
regions <- convertSample(chunk.data$regions)
...
for(sample.id in names(profile.list)){
  ...
  sorted.regions <- region.list[[sample.id]]
  seconds <- system.time({
  ...
  for(peaks.str in names(fit$fit$peaks)){
    ...
    error <- PeakErrorChrom(peaks.df, sorted.regions)
...
for(bases.per.problem.i in seq_along(bases.per.problem.vec)){
  ...
  problem.name <- sprintf("chr11:%d-%d", problemStart, problemEnd)
  problems <- 
...
ggplot()+
  ...
  geom_text(aes(max(best.problems$problemEnd)/1e3, 1,
...
png("figure-H3K27ac-profiles.png", width=9, h=8, res=200, units="in")
print(problemsPlot)
dev.off()
...
print(single.times.xt, include.rownames=FALSE, floating=FALSE,
      file="table-H3K27ac-PeakSeg.tex")

xt <- xtable(best.times.df)
print(xt, file="table-H3K27ac.tex", floating=FALSE)
