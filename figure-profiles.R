...
flat.segs <- function(sample.ids){
  data.frame(chromStart=max.chromStart,
             ...
             mean=flat.means[sample.ids],
             sample.id=sample.ids,
             model.i=NA)
}
zoom.peak.list <- list("0"=Peaks())
zoom.seg.list <-
  list("0"=flat.segs(names(profile.list)))
zoom.loss.list <-
...
for(peaks.str in names(best.indices.list)){
  ...
  peaks <- as.numeric(peaks.str)
  segs.with.peaks.names <- paste(best.model$model.i, 1:3)
  segs.with.peaks <- do.call(rbind, seg.list[segs.with.peaks.names])
  segs.without.peaks <- if(length(samples.without.peaks)){
    flat.segs(samples.without.peaks)
  }
  zoom.segs <- rbind(segs.without.peaks, segs.with.peaks)
  zoom.seg.list[[peaks.str]] <- zoom.segs
